...
import { signInDemoUser } from "@/lib/auth/demo-sign-in";
...
export default function LoginPage() {
  ...
  async function completeSignIn(propertySlug = "harbour-view") {
    if (redirectingRef.current) return;
    redirectingRef.current = true;
    router.replace(resolvePostLoginPath(callbackUrl, propertySlug));
  }
  ...
  async function signInWithCredentials(
    ...
    await completeSignIn(propertySlug);
  }
  ...
  async function onDemoLogin(user: (typeof DEMO_USERS)[number]) {
    ...
    setState("loading");
    try {
      const result = await signInDemoUser(user.email, user.slug, callbackUrl ?? undefined);
      if (!result.ok) {
        redirectingRef.current = false;
        setState("invalid");
        return;
